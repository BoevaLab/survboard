...
for (project in c("validation")) {
  for (cancer in c("PAAD", "LIHC")) {
    ...
    for (split in 1:25) {
      ...
      combined_data <- dummy_cols(rbind(data_input[train_ix, ], data_transfer_to)[, -c(which("OS" == colnames(data_input)), which("OS_days" == colnames(data_input)))], remove_first_dummy = TRUE, remove_selected_columns = TRUE)
      train_data <- (combined_data[1:length(train_ix), ])
      test_data <- (combined_data[-(1:length(train_ix)), ])
      ...
      survival_function %>% write_csv(
        here::here(
          "results_reproduced", "survival_functions", "transfer", "validation", cancer, "multimodal_nsclc", paste0("split_", split, ".csv")
        )
      )
    }
  }
