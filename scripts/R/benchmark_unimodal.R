...
for (modality in c("clinical", "gex", "mirna", "mut", "meth", "cnv", "rppa")) {
  for (project in c("METABRIC", "TCGA", "ICGC", "TARGET")) {
    ...
    for (cancer in config[[paste0(tolower(project), "_cancers")]]) {
      if (!modality %in% config[[paste0(tolower(project), "_modalities")]][[cancer]]) {
        # Skip if this modality is not available for this cancer type.
        next
      }
      print(paste0("Starting: ", project, "-", cancer))
      ...
      data <- data[
        ,
        which(sapply(strsplit(colnames(data), "\\_"), function(x) x[[1]]) %in% c("OS", modality))
      ]
      ...
      for (model in 0:(length(model_names) - 1)) {
        if (!dir.exists(here::here(
          "results_reproduced", "survival_functions", modality, project, cancer, model_names[(model + 1)]
        ))) {
          dir.create(
            here::here(
              "results_reproduced", "survival_functions", modality, project, cancer, model_names[(model + 1)]
        ...
        for (i in 1:(config$outer_repetitions * config$outer_splits)) {
          data.frame(bmr[((1 + (model * (config$outer_repetitions * config$outer_splits))):((model + 1) * (config$outer_repetitions * config$outer_splits)))]$prediction[[i]]$data$distr, check.names = FALSE) %>% readr::write_csv(
            here::here(
              "results_reproduced", "survival_functions", modality, project, cancer, model_names[(model + 1)], paste0("split_", i, ".csv")
            )
          )
        }
      }
