...
multimodal_metrics <- multimodal_metrics %>%
  #filter(!grepl("(discrete_time|early)", model)) %>%
  group_by(project, cancer, model, modalities) %>%
  mutate(d_calibration = replace_na(d_calibration, mean(d_calibration, na.rm = TRUE))) %>%
  ungroup()
...
ggsave("./figures_reproduced/survboard_final_fig_S1.pdf", width = 20, height = 8, plot = a)
ggsave("./figures_reproduced/survboard_final_fig_S1.svg", width = 20, height = 8, plot = a)
