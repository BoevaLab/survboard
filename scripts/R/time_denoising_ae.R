  ...
  for (cancer in c(options[2])) {
    ...
    data <- vroom::vroom(
      here::here(
        ...
        paste0(snakemake@wildcards[["cancer"]], "_data_complete_modalities_preprocessed.csv", collapse = "")
      )
    )
    ...
    zero_variance_mask <- (apply(train_data, 2, var) == 0)
    ...
    for (modality in c("gex", "clinical")) {
      ...
      if (modality == "gex" & "gex" %in% modalities) {
        ...
        tmp <- linear_featureselection_func(
          scale(mod_train_vared), NULL, train_label$OS_days, train_label$OS, min(n_other, ncol(mod_train))
        )
      ...
      if (modality == "mirna" & "mirna" %in% modalities) {
        ...
        mod_train_vared <- mod_train[, apply(mod_train, 2, var) > median(apply(mod_train, 2, var))]
        tmp <- linear_featureselection_func(
          scale(mod_train_vared), NULL, train_label$OS_days, train_label$OS, min(n_mirna, ncol(mod_train))
        )
      ...
      if (modality == "rppa" & "rppa" %in% modalities) {
        ...
        mod_train_vared <- mod_train[, apply(mod_train, 2, var) > median(apply(mod_train, 2, var))]
        tmp <- linear_featureselection_func(
          scale(mod_train_vared), NULL, train_label$OS_days, train_label$OS, min(n_rppa, ncol(mod_train))
        )
        transformed_matrix_train <- cbind(transformed_matrix_train, tmp[[1]])
      }
    ...
    ae_projections <- denoising_zeros_func(transformed_matrix_train, NULL, fullAE_features)
    ae_projections_train <- ae_projections[[1]]

    survival_function <- elastic_net_func(ae_projections_train, NULL, train_label$OS_days, train_label$OS)
    write.table(data.frame(), file = here::here(
      "results_reproduced", "timings", paste0("denoising_ae_", snakemake@wildcards[["cancer"]])
    ), col.names = FALSE)
  }
}
