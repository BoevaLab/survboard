                ...
                for cancer in ["PAAD", "LIHC"]:
                    ...
                    ct = ColumnTransformer(
                        [
                            (
                                "numerical",
                                make_pipeline(
                                    VarianceThreshold(threshold=0.01), MinMaxScaler()
                                ),
                    ...
                    X_test = pd.concat(
                        [
                            pd.DataFrame(
                                ct.transform(
                                    predict_data.drop(columns=["OS", "OS_days"])
                                ),
                                columns=(
                    ...
                    for outer_split in range(25):
                        ...
                        samples_train, samples_val = train_test_split(
                            ...
                            stratify=clinical_df[["OS"]].values[
                                np.array(samples_train)
                            ],
                        )
                        ...
                        val_risk = (
                            model.predict_risk(omics_val).detach().numpy().ravel()
                        )
                        test_risk = (
                            model.predict_risk(omics_test).detach().numpy().ravel()
                        )

                        train_time = (
                            clinical_df[["OS_days"]].values[samples_train].ravel()
                        )
                        val_time = clinical_df[["OS_days"]].values[samples_val].ravel()
                        test_time = (
                            clinical_df[["OS_days"]].values[samples_test].ravel()
                        )

                        train_event = clinical_df[["OS"]].values[samples_train].ravel()
                        val_event = clinical_df[["OS"]].values[samples_val].ravel()
                        test_event = clinical_df[["OS"]].values[samples_test].ravel()
                        ...
                        survival_functions = be.get_survival_function(test_risk)
