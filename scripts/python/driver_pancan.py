...
def main(model_type, fusion):
    ...
    data_path = f"./data_reproduced/TCGA/pancancer_complete.csv"
    data = pd.read_csv(
    ...
    data = data.iloc[
        ...
        [
            ...
            if data.columns[i].rsplit("_")[0] in ["clinical", "gex", "OS"]
        ],
    ]
    data_helper = data.copy(deep=True).drop(columns=["OS_days", "OS"])
    train_splits = pd.read_csv(
        os.path.join(f"./data_reproduced/splits/TCGA/pancancer_train_splits.csv")
    )
    test_splits = pd.read_csv(
        os.path.join(f"./data_reproduced/splits/TCGA/pancancer_test_splits.csv")
    )
    for outer_split in range(config["outer_repetitions"] * config["outer_splits"]):
        ...
        X_train = pd.DataFrame(
            ...
            + [
                f"clinical_{i}"
                for i in ct.transformers_[1][1][0].get_feature_names_out().tolist()
            ],
        )
        ...
        if model_type == "cox":
            ...
            survival_probabilities = np.stack(
                [i(np.unique(np.abs(y_train))) for i in survival_functions]
            )
        ...
        if np.any(np.isnan(sf_df)):
            ...
            sf_df = pd.DataFrame([survival_km for i in range(X_test.shape[0])])
