...
for (cancer in config$tcga_cancers) {
  ...
  ) %>% dplyr::rename(patient_id = clinical_patient_id)
  ...
  data <- vroom::vroom(
    here::here(
      "~", "boeva_lab_scratch", "data", "projects", "David", "Nikita_David_survival_benchmark",
      "survival_benchmark", "data", "processed", "TCGA",
      paste0(cancer, "_data_non_complete_modalities_preprocessed.csv", collapse = "")
    )
  )
...
library(data.table)
complete_pancancer <- data.table::rbindlist(lapply(complete_list, function(x) x[, joint_features_all]))
incomplete_pancancer <- data.table::rbindlist(lapply(incomplete_list, function(x) x[, joint_features_all]))
...
complete_pancancer %>% write_csv(
  here::here(
    "~", "boeva_lab_scratch", "data", "projects", "David", "Nikita_David_survival_benchmark",
    "survival_benchmark", "data", "processed", "pancancer_complete.csv"
  )
)
...
incomplete_pancancer %>% write_csv(
  here::here(
    "~", "boeva_lab_scratch", "data", "projects", "David", "Nikita_David_survival_benchmark",
    "survival_benchmark", "data", "processed", "pancancer_incomplete.csv"
  )
)
