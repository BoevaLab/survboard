...
filter_out_duplicates_icgc <- function(df) {
  ...
  # preprocess this stuff first, leaving only n_specimens == 1 and n_samples > 1
  duplicated_specimen_and_sample_donors <- indicator_frame %>%
    filter(n_samples > n_specimens & n_specimens > 1) %>%
    pull(icgc_donor_id)
  
  if(length(duplicated_specimen_and_sample_donors) > 0) {
    print("ATTENTION!!! DOUBLE CHECK ME!!!")
  }
  ...
  # Handle duplicated specimens first
  ...
  # Now, filter out those specimens
  ...
  # Handle duplicated samples next
...
prepare_gex_icgc <- function(gex, type = "seq", log = TRUE) {
  ...
  if (type == "seq") {
    ...
    print("HELLO PIVOT MY OLD FRIEND")
    gex <- gex %>%
...
prepare_icgc <- function(cancer) {
  ...
  sample <- vroom::vroom(
    here::here(
      "~", "boeva_lab_scratch", "data", "projects", "David", "Nikita_David_survival_benchmark", "survival_benchmark", "data", "raw", "ICGC", paste0("sample.", cancer, ".tsv.gz")
    )
  )
  specimen <- vroom::vroom(
    here::here(
      "~", "boeva_lab_scratch", "data", "projects", "David", "Nikita_David_survival_benchmark", "survival_benchmark", "data", "raw", "ICGC", paste0("specimen.", cancer, ".tsv.gz")
    )
  )
  donor <- vroom::vroom(
    here::here(
      "~", "boeva_lab_scratch", "data", "projects", "David", "Nikita_David_survival_benchmark", "survival_benchmark", "data", "raw", "ICGC", paste0("donor.", cancer, ".tsv.gz")
    )
  ) %>%
    mutate(
  ...
  if (config$exposure[[cancer]]) {
    exposure <- vroom::vroom(
      here::here(
        "~", "boeva_lab_scratch", "data", "projects", "David", "Nikita_David_survival_benchmark", "survival_benchmark", "data", "raw", "ICGC", paste0("donor_exposure.", cancer, ".tsv.gz")
      )
    ) %>%
  ...
  if ("gex" %in% config$icgc_modalities[[cancer]]) {
    ...
    if (cancer == "LICA-FR") {
      gex <- vroom::vroom(
        here::here(
          "~", "boeva_lab_scratch", "data", "projects", "David", "Nikita_David_survival_benchmark", "survival_benchmark", "data", "raw", "ICGC", paste0("exp_seq", ".", cancer, "_corrected.tsv.gz")
        )
      )
    } else if (config$gex_type[[cancer]] == "seq") {
      gex <- vroom::vroom(
        here::here(
          "~", "boeva_lab_scratch", "data", "projects", "David", "Nikita_David_survival_benchmark", "survival_benchmark", "data", "raw", "ICGC", paste0("exp_seq", ".", cancer, ".tsv.gz")
        )
      )
    } else {
      gex <- vroom::vroom(
        here::here(
          "~", "boeva_lab_scratch", "data", "projects", "David", "Nikita_David_survival_benchmark", "survival_benchmark", "data", "raw", "ICGC", paste0("exp_array", ".", cancer, ".tsv.gz")
        )
      )
    }
  ...
  if ("mutation" %in% config$icgc_modalities[[cancer]]) {
    print("BEGINNING MUTATION")
    mut <- maftools::icgcSimpleMutationToMAF(
      here::here(
        "~", "boeva_lab_scratch", "data", "projects", "David", "Nikita_David_survival_benchmark", "survival_benchmark", "data", "raw", "ICGC", paste0("simple_somatic_mutation.open", ".", cancer, ".tsv.gz")
      ),
      MAFobj = TRUE
    )
  ...
  data %>%
    ...
    write_csv(paste0("~/boeva_lab_scratch/data/projects/David/Nikita_David_survival_benchmark/survival_benchmark/data/processed/ICGC/", paste0(cancer, "_data_complete_modalities_preprocessed.csv")))

  incomplete_data %>%
    dplyr::rename(patient_id = icgc_donor_id) %>%
    write_csv(paste0("~/boeva_lab_scratch/data/projects/David/Nikita_David_survival_benchmark/survival_benchmark/data/processed/ICGC/", paste0(cancer, "_data_non_complete_modalities_preprocessed.csv")))
}
