...
def negative_partial_log_likelihood(
    ...
    predicted_log_hazard_ratio = torch.clamp(predicted_log_hazard_ratio, -75, 75)
    if torch.sum(observed_event_indicator) <= 0.0:
        return torch.tensor(0.0, requires_grad=True)
    #print(observed_survival_time.shape)
...
def sparse_group_lasso(weights, lamb, alpha, groups):
    ...
    return lamb * (1 - alpha) * lasso_penalty + lamb * alpha * group_lasso_penalty


def lasso_all(parameters, lamb):
    lasso_penalty = 0.0
    for param in parameters:
        lasso_penalty += lamb * torch.norm(input=param, p=1)
...
class BreslowLassoLoss(_Loss):
    def __init__(self, size_average=None, reduce=None, reduction: str = "mean") -> None:
        super().__init__(size_average, reduce, reduction)

    def forward(self, prediction, input, lamb, parameters):
        time, event = transform_back_torch(input)
        # print("----")
        # print(negative_partial_log_likelihood(prediction, time, event))
        # print((1 / torch.sum(event)) * lasso_all(parameters=parameters, lamb=1.0))
        # print("----")
        loss = negative_partial_log_likelihood(prediction, time, event) + (
            (1 / torch.sum(event)) * lasso_all(parameters=parameters, lamb=lamb)
        )
        return loss
...
def get_blocks_salmon(feature_names):
    column_types = pd.Series(feature_names).str.rsplit("_").apply(lambda x: x[0]).values
    proper_blocks = [
        np.where(
            modality
    ...
    low_dimensional_blocks = [x for xs in low_dimensional_blocks for x in xs]
    return proper_blocks + [low_dimensional_blocks]


def get_blocks_gdp(feature_names):
    column_types = pd.Series(feature_names).str.rsplit("_").apply(lambda x: x[0]).values
    modality_blocks = [
        np.where(
            modality
    ...
    gene_level_modalities = [
        (
            ...
            + [
                i
                for i in feature_names[gex_ix]
                if i.rsplit("|")[0].rsplit("_")[1] == gene
            ]
        )
        for gene in genes
    ]
    gene_level_modalities_ix = [
        [
            np.where(feature_name == feature_names)[0][0].tolist()
            for feature_name in gene
        ]
        for gene in gene_level_modalities
    ]

    return modality_blocks + gene_level_modalities_ix
