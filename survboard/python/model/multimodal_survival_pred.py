...
class Attention(nn.Module):
    def __init__(self, m_length, modalities, device=None):
        ...
        for modality in self.data_modalities:
            self.pipeline[modality] = nn.Linear(
                self.m_length, self.m_length, bias=False
            ).to(self.device)
    ...
    def forward(self, multimodal_input):
        ...
        for modality in self.data_modalities:
            attention_weight += (
                torch.tanh(self.pipeline[modality](multimodal_input[modality])),
            )
        ...
        fused_vec = self._scale_for_missing_modalities(
            torch.stack(multimodal_features), fused_vec
        )
        return fused_vec
...
class FixedAttention(nn.Module):
    ...
    def forward(self, multimodal_input):
        ...
        for modality in self.data_modalities:
            attention_weight += (
                torch.ones(multimodal_input[modality].shape[0], self.m_length).to(
                    self.device
                ),
            )
            multimodal_features += (multimodal_input[modality],)
...
class Loss(nn.Module):
    ...
    def _contrastive_loss1(
        self, x1_idx, x2_idx, representation, modalities, margin=0.2
    ):
        """
        Only one modality
        """
        ...
        for idx1, idx2 in zip(x1_idx, x2_idx):
            dis_x_y = torch.cosine_similarity(
                representation[modality][idx1], representation[modality][idx2], dim=0
            )
            con_loss += torch.pow(torch.clamp(margin + dis_x_y, min=0.0), 2)

        return con_loss / len(x1_idx)

    def _contrastive_loss2(
        self, x1_idx, x2_idx, representation, modalities, margin=0.2
    ):
        """
        ...
        for idx1, idx2 in zip(x1_idx, x2_idx):
            ...
            for i in range(len(modalities) - 1):
                for j in range(i + 1, len(modalities)):
                    dis_x_x += torch.cosine_similarity(
                        representation[modalities[i]][idx1],
                        representation[modalities[j]][idx1],
                        dim=0,
                    )
                    dis_y_y += torch.cosine_similarity(
                        representation[modalities[i]][idx2],
                        representation[modalities[j]][idx2],
                        dim=0,
                    )
            dis_x_y = 0
            for modality in modalities:
                dis_x_y += torch.cosine_similarity(
                    representation[modality][idx1],
                    representation[modality][idx2],
                    dim=0,
                )
            con_loss += torch.pow(
                torch.clamp(margin + dis_x_y - 0.5 * dis_x_x - 0.5 * dis_y_y, min=0.0),
                2,
            )
    ...
    def _unsupervised_similarity_loss(self, representation, modalities, t=1):
        ...
        if len(modalities) > 1:
            while k < t:
                x1_idx, x2_idx = self._random_match(
                    representation[modalities[0]].shape[0]
                )
                similarity_loss += self._contrastive_loss2(
                    x1_idx, x2_idx, representation, modalities
                )
                k += 1
        else:
            while k < t:
                ...
                similarity_loss += self._contrastive_loss1(
                    x1_idx, x2_idx, representation, modalities
                )
                k += 1
    ...
    def forward(self, representation, modalities, pred_hazard, event, time):
        ...
        if self.mode == "total":
            loss = (
                self._cross_entropy_loss(pred_hazard, event)
                + self._negative_log_likelihood_loss(pred_hazard, event, time)
                + self.trade_off
                * self._unsupervised_similarity_loss(representation, modalities)
            )
...
class Net(nn.Module):
    def __init__(
        ...
        if "clinical" in self.data_modalities:
            self.clinical_submodel = ClinicalEmbeddingNet(
                m_length=self.m_length, clinical_length=self.dim["clinical"]
            )
            self.submodel_pipeline["clinical"] = self.clinical_submodel

        # mRNA ---------------------------------------------------#
        if "mRNA" in self.data_modalities:
            self.mRNA_submodel = MrnaNet(
                mrna_length=self.dim["mRNA"], m_length=self.m_length
            )
            self.submodel_pipeline["mRNA"] = self.mRNA_submodel

        # meth ---------------------------------------------------#
        if "meth" in self.data_modalities:
            self.meth_submodel = MrnaNet(
                mrna_length=self.dim["meth"], m_length=self.m_length
            )
            self.submodel_pipeline["meth"] = self.meth_submodel

        # mut ---------------------------------------------------#
        if "mut" in self.data_modalities:
            self.mut_submodel = CnvNet(
                cnv_length=self.dim["mut"], m_length=self.m_length
            )
            self.submodel_pipeline["mut"] = self.mut_submodel

        # miRNA --------------------------------------------------#
        if "miRNA" in self.data_modalities:
            self.miRNA_submodel = MirnaNet(
                mirna_length=self.dim["miRNA"], m_length=self.m_length
            )
            self.submodel_pipeline["miRNA"] = self.miRNA_submodel

        if "rppa" in self.data_modalities:
            self.rppa_submodel = MirnaNet(
                mirna_length=self.dim["rppa"], m_length=self.m_length
            )
            self.submodel_pipeline["rppa"] = self.rppa_submodel

        # CNV ----------------------------------------------------#
        if "CNV" in self.data_modalities:
            self.CNV_submodel = CnvNet(
                cnv_length=self.dim["CNV"], m_length=self.m_length
            )
    ...
    def forward(self, x):
        ...
        for modality in x:
            if modality in ["clinical_categorical", "clinical_continuous"]:
                if flag:
                    representation["clinical"] = self.submodel_pipeline["clinical"](
...
class ModelCoach:
    def __init__(
        self, model, modalities, dataloaders, optimizer, criterion, device=None
    ):
        self.model = model
    ...
    def _log_info(self, phase, logger, epoch, epoch_loss, epoch_c_index):
        info = {phase + "_loss": epoch_loss, phase + "_c_index": epoch_c_index}
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            for phase in ["train", "val"]:
                ...
                for data, data_label in self.dataloaders[phase]:
                    loss, hazard, time, event = self._process_data_batch(
                        data, data_label, phase
                    )

                    running_loss.append(loss.item())
                    running_sample_time = torch.cat(
                        (running_sample_time, time.data.float())
                    )
                    running_sample_event = torch.cat(
                        (running_sample_event, event.long().data)
                    )
                ...
                if phase == "val":
                    ...
                    if epoch_c_index > self.best_perf["best_score"]:
                        ...
                        self.best_wts["best_wts"] = copy.deepcopy(
...
class Model(_BaseModelWithDataLoader):
    def __init__(
        ...
        super().__init__(
            modalities, m_length, dataloaders, fusion_method, device, input_modality_dim
        )
    ...
    def fit(
        self, num_epochs, lr, info_freq, log_dir, lr_factor=0.1, scheduler_patience=5
    ):
        self._instantiate_model()
    ...
    def save_weights(self, saved_epoch, prefix, weight_dir):
        ...
        if saved_epoch == "current":
            ...
            file_name = os.path.join(
                weight_dir, f"{prefix}_{epoch}_c_index{value:.2f}.pth"
            )
        else:
            file_name = os.path.join(
                weight_dir,
...
def get_dataloaders(mydataset, train_sampler, val_sampler, test_sampler, batch_size):
    ...
    dataloaders["train"] = DataLoader(
        mydataset, batch_size=batch_size, sampler=SubsetRandomSampler(train_sampler)
    )
    dataloaders["val"] = DataLoader(
        mydataset, batch_size=len(val_sampler), sampler=SubsetRandomSampler(val_sampler)
    )
    dataloaders["test"] = DataLoader(
        mydataset, batch_size=len(test_sampler), sampler=test_sampler
    )
...
def preprocess_clinical_data(clinical_df):
    ...
    clin_data_continuous = clinical_df.iloc[
        :, np.where(clinical_df.dtypes != "object")[0]
    ]
    clin_data_categorical = clinical_df.iloc[
        :, np.where(clinical_df.dtypes == "object")[0]
    ]
