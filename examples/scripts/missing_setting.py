...
parser.add_argument(
    "config_path",
    type=str,
    help="Path to the parameters needed for training in JSON format. Also includes the list of cancers available for each project. To experiment for a few cancers, simply alter this list accordingly.",
)
...
def main(
    ...
    logging.basicConfig(
        handlers=[
            logging.FileHandler(os.path.join(save_here, "missing_experiment.log")),
    ...
    for cancer in cancers:
        ...
        # Load and process data with incomplete/missing modalities
        ...
        for outer_split in range(25):
            ...
            train_base_ix, train_ix, test_ix = get_splits(
                data_dir, cancer, project, n_samples=data_combined.shape[0], split_number=outer_split, setting="missing"
            )
